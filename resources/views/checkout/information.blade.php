...
@section('content')
    ...
    <main>
        ...
        <div class="container-fluid">
            ...
            {{ Form::open(['route' => 'client-checkout-shipping', 'method' => 'get', 'id' => 'submitForm']) }}
            <div class="row justify-content-center mb-5 pb-5">
                ...
                <div class="col-lg-5 px-5 my-3 checkout-form">
                    <h3 class="fw-bold mb-5 fst-italic"><b>BILLING INFORMATION</b></h5>
                        ...
                        <p style="color: #707070">Default Contact Number or <a href="#" class="updateNum"
                                ...
                                style="color: #EA5656"><strong>@if (Auth::user()->contact == '') Add Number @else Update Number @endif </strong></a></p>
                        <input type="text" name="contact_number" class="form-control mt-2" style="color: grey"
                            value="{{ Auth::user()->contact }}" required readonly="true">
                            ...
                            <div class="col">
                                <select name="billing_id" class="form-control" required="true">
                                    @foreach ($billingAddress as $x)
                                        <option value="{{ $x->id }}" @if ($x->id == $billingAddress->last()->id) selected @endif>{{ $x->address }}
                                            {{ $x->city }} {{ $x->zip }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="row mt-4 align-items-center">
                            <div class="col-lg-6 my-2">
                                <small><a href="{{ route('client-view-cart') }}"
                                        class="link-main-custom align-middle">RETURN TO CART</a></small>
                            </div>
                            <div class="col-lg-6 my-2">
                                <input type="submit" value="CONTINUE TO SHIPPING" class="btn btn-shop"
                                    onclick="this.disabled=true;this.value='Submitted, please wait...';document.getElementById('submitForm').submit();">
                            </div>
                        </div>
                </div>
                <!-- Right Section -->
                <div class="col-lg-5 px-5 my-3">
                        ...
                        <table class="table table-borderless">
                            <tbody>
                                @foreach (\Cart::session(Auth::user()->id)->getContent() as $x)
                                    <tr>
                                        <td>
                                            <img src="/uploads/products/{{ App\Product::find($x->id)->getProductImage() }}"
                                                class="shipping-img mr-3" style="float: left;width:75px;height:75px">
                                        </td>
                                        <td class="w-50">
                    ...
                    <hr>
                    <p class="px-2"><span class="text-bold">SUBTOTAL</span><span class="float-end">₱
                            {{ number_format(\Cart::session(Auth::user()->id)->getSubTotal(), 2) }}</span></p>
                </div>
            </div>
            </form>
        </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        {{ Form::open(['route' => ['account.billing-address.store-modal', $user->id], 'method' => 'store', 'files' => true]) }}
                        <div class="row">
                            <div class="col-lg-12">
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                ...
                                {{ Form::text('brgy', '', ['class' => 'form-control span6', 'placeholder' => 'Barangay', 'required' => 'required']) }}
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                ...
                                {{ Form::text('region', '', ['class' => 'form-control span6', 'placeholder' => 'Metro Manila/NCR', 'required' => 'required']) }}
                                <span class="errors" style="color:#FF0000">{{ $errors->first('region') }}</span>
                            </div>
                            <div class="col-lg-6">
                                {{ Form::label('city', 'City') }}
                                {{ Form::text('city', '', ['class' => 'form-control span6', 'placeholder' => 'City', 'required' => 'required']) }}
        ...
        <div class="modal fade" id="myModalNumber" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        {{ Form::open(['route' => ['account.contact.store-modal', $user->id], 'method' => 'store', 'files' => true]) }}
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                ...
                                <input type="text" class="form-control" placeholder="Contact Number" name="contact_number"
                                    data-mask data-mask-format="+63 999 999 9999">
                                {{-- {{ Form::number('contact','',array('class'=>'form-control span6','placeholder' => 'Contact No.','required'=>'required')) }} --}}
                            </div>
                        </div>
                        <br />
                        <div class="row top10">

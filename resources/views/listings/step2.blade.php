                ...
                <div class="col-lg-9" id="cart-col">
                    <h2>Rent&nbsp;<b>{{ $rental->bike_unit }}</b></h2>
                        ...
                        <form method="post" action="{{ route('client-rental-payment') }}">
                            ...
                            <div class="row">
                                <div class="col-lg-3 col-md-4 col-12 pt-4 me-5">
                                    <div class="card card-course images">
                                        <div class="card-body">
                                            <div class="col img-modal"
                                                ...
                                                data-src="{{ asset('uploads/rentals/' . $rental->getProductImage()) }}">
                                            </div>
                                        ...
                                        <div class="card card-course-foot shadow border-0">
                                            <a class="btn btn-background text-light mx-auto fst-italic btn border-0 fs-5 p-2  w-100 card-course-foot"
                                                href="{{ route('rental-detail', $rental->id) }}">Other Details</a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-8 col-md-4 col-12 pt-4 ms-3">
                                    <div class="col-lg-4">
                                        {{ Form::label('dt_from', 'From') }}
                                        <p>DATE: {{ Carbon\Carbon::parse($dt_from . ' ' . $dt_from_time) }}</p>
                                    </div>
                                    <div class="col-lg-4">
                                        {{ Form::label('dt_to', 'To') }}
                                        <p>DATE: {{ Carbon\Carbon::parse($dt_to . ' ' . $dt_to_time) }}</p>
                                    </div>
                                    <div class="col-lg-4">
                                        <p>Duration</p>
                                        <p> {{ Carbon\Carbon::parse($dt_to . ' ' . $dt_to_time)->diffInDays(Carbon\Carbon::parse($dt_from . ' ' . $dt_from_time)) }}
                                            Days</p>
                                    </div>
                                </div>
                            </div>
                            {{-- <table class="table table-condensed">
                                ...
                                <tbody>
                                    <tr>
                                        <td>
                                            <img src="/uploads/rentals/{{ App\Rentals::find($rental->id)->getProductImage() }}"
                                                style="width:100px;height:100px;float:left;padding-right: 0px;margin-right: 15px;object-fit:cover;"><p><strong>
                                                    <a href="/product/{{ App\Rentals::find($rental->id)->first()->id or 0 }}"
                                                        style="color: #F2B533;">{{ $rental->name }}</a></strong></p>
                                        </td>
                            ...
                            <input class="mt-4 btn-background border-0 rounded p-2 fw-bold" type="submit" name="CHECKOUT">
                        </form>    
                    </div>
                </div>
            </div>
